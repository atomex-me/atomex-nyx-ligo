type transferFromParam is list ((nat * address) * address);
type transferParam is list (nat * address);
...
function doInitiate(const initiate: initiateParam; var s: storage) : (list(operation) * storage) is 
  block {
    ...
    const transferFromEntry: contract(transferFromParam) = getTransferFromEntry(initiate.tokenAddress);
    const depositTx: operation = transferFrom(
      transferFromEntry, Tezos.sender, Tezos.self_address, initiate.totalAmount);
...
function doRedeem(const secret: bytes; var s: storage) : (list(operation) * storage) is
  block {
    ...
    const redeemTx: operation = transfer(transferEntry, swap.participant, swap.totalAmount);
...
function doRefund(const hashedSecret: bytes; var s: storage) : (list(operation) * storage) is
  block {
    ...
    const transferEntry: contract(transferParam) = getTransferEntry(swap.tokenAddress);
    const refundTx: operation = transfer(transferEntry, swap.initiator, swap.totalAmount);
